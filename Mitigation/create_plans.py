...
login("{huggingface_token}")  # NOTE: replace with your Hugging Face token if needed for private models

# ===========================
# Config
# ===========================
PLAN_DIR = "output/created_plans" #NOTE"{directory for generated plans}"  # plans generated by the trained model
ROOT_CAUSE_LOCATION = "output/eval_rootcauses" #NOTE"output/{directory for root cases for evaluation}"#"adservice_cpu_1_baro_re1_ob.json"  # JSON with list of top 10 root causes
OUTPUT_CSV = "llm_judged_evaluation" #NOTE"{filename for saving evaluation results}"  # CSV file to save the evaluation results


ALL_CRITERIA=("scalability" "clarity")

ACTIVE_CRITERIA=("json_structure" "efficiency")

TRAINED_MODELS = [
    #NOTE--> open source models for creating plans 
...
def build_mitigation_plan(trained_model_name, generator, root_cause_location=ROOT_CAUSE_LOCATION):
    mitigation_format = {
    ...
    #get files under root cause location
    root_cause_files = [f for f in os.listdir(root_cause_location)]

    # Generate model output (try 3 times, each with different seed)
    print(root_cause_files)
    for file_num,file_name in enumerate(root_cause_files):
        prompt = (
        ...
        # Load root causes
        with open(os.path.join(root_cause_location, file_name), "r") as f:
        ...
        os.makedirs(f"output/{PLAN_DIR}", exist_ok=True)
        output_path = f"output/{PLAN_DIR}/{trained_model_name}/plan_seed_{file_num}.json"
